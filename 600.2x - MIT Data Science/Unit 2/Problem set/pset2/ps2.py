...
class RectangularRoom(object):
    ...
    def getRandomPosition(self):
        ...
        """
        return Position(round(random.choice(arange(0, self.width + 0.01, 0.01)), 2), round(random.choice(arange(0, self.height + 0.01, 0.01)),2))
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        been cleaned.
        """
        try_pos = self.position.getNewPosition(self.getRobotDirection(), self.speed)
        if self.room.isPositionInRoom(try_pos):
            super().updatePositionAndClean()
        else:
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    robots = [robot_type(test_room, speed) for _ in range(num_robots)]
    time_steps = []
    for _ in range(num_trials):
        # anim = ps2_visualize.RobotVisualization(num_robots, width, height)
        percent_clean = test_room.getNumCleanedTiles() / test_room.getNumTiles()
        clock_ticks = 0
        while percent_clean < min_coverage:
            for robot in robots:
                robot.updatePositionAndClean()
                # anim.update(test_room, robots)
            clock_ticks += 1
            percent_clean = test_room.getNumCleanedTiles() / test_room.getNumTiles()
        time_steps.append(clock_ticks)
        # anim.done()
    return mean(time_steps)
...
#     plot.
#
showPlot1(f"Cleaning Simulation - num of robots variations", "no. robots", "time per robot type")
#
...
#     plot.
#
showPlot2(f"Cleaning Simulation, 2 robots - room variations", "aspect ratio", "time per robot type")
#
